...
class JavaClassSpecs(relPath: String, absPaths: Seq[String], firstSpec: ClassSpec)
  ...
  override def importUrl(url: String, path: List[String], inFile: Option[String]): Future[Option[ClassSpec]] = Future {
    Log.importOps.info(() => s".. importing from URL $url")
    JavaClassSpecs.cached(path, inFile, urlFiles, url, downloadFromUrl)
  }
  ...
  def downloadFromUrl(urlString: String): ClassSpec = {
    Log.importOps.info(() => s".... downloading from $urlString")
    ...
    try {
      ...
      if (responseCode != 200) {
        throw new IOException(s"HTTP error code: $responseCode for URL: $urlString")
      }
